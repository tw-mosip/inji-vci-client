...
- **Structured error handling** - `VCIClientException` now carries `sourceErrorCode`, `serverErrorCode`, and `serverErrorDescription` alongside `code` and `message`, so consumers can distinguish library failures from issuer/authorization-server error payloads (see [Error Handling](#-error-handling)).
...
- If you want to infer server-side failures, use `serverErrorCode` and `serverErrorDescription` rather than parsing `message`.
